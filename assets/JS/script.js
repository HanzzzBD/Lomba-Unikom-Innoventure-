...
function initScrollJacking() {
    ...
    const container = document.querySelector('.scroll-jacking-container');
    
    if (!jurusanSection || !cards.length || !container) return;
    ...
    function handleScroll() {
        ...
        const sectionTop = jurusanSection.offsetTop;
        ...
        const cardIndex = Math.floor(sectionProgress * cards.length);
        
        if (cardIndex !== currentCardIndex && cardIndex >= 0 && cardIndex < cards.length) {
            currentCardIndex = Math.max(0, Math.min(cardIndex, cards.length - 1));
            updateCardStates();
        }

        // Parallax effect for background
        const parallaxBg = document.querySelector('.jurusan-section-bg');
        if (parallaxBg) {
            const translateY = (scrollY - sectionTop) * 0.5;
            parallaxBg.style.transform = `translateZ(-1px) scale(2) translateY(${translateY}px)`;
        }
    }
    ...
    function updateCardStates() {
        cards.forEach((card, index) => {
            ...
            card.classList.remove('active', 'next', 'prev', 'far', 'stacked');
    ...
    function scrollToCard(index) {
        ...
        const sectionTop = jurusanSection.offsetTop;
        const targetScroll = sectionTop + cardTop - window.innerHeight / 2;
    ...
    cards.forEach((card, index) => {
        ...
        card.setAttribute('aria-label', `Scroll to ${card.querySelector('h3').textContent}`);
    });
    ...
    container.addEventListener('wheel', (e) => {
        ...
        const newIndex = Math.max(0, Math.min(currentCardIndex + direction, cards.length - 1));
...
// Initialize card reveal on page load
